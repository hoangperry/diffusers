...
class TransformerBlockMetadata:
    ...
    def _register(self, cls):
        ...
        Lets ``@register_metadata(TransformerBlockMetadata(...))`` work for block classes that
        opt into the decorator pattern (e.g. Flux). Models that use the legacy bulk registration
        in ``_register_transformer_blocks_metadata`` are unaffected — both code paths call the
        same ``TransformerBlockRegistry.register`` underneath.
        """
